...
// Creating a Bank Account Interface
interface BankAccount {
  accountNumber: number; // Unique account number
  balance: number; // Balance of the account
  deposit(amount: number): void; // Method to deposit money
  withdraw(amount: number): void; // Method to withdraw money
  checkBalance(): void; // Method to check balance
}
...
class BankAccountClass implements BankAccount {
  ...
  deposit(amount: number): void {
    ...
    console.log(
      `Deposited ${amount} into your account. Your new balance is: ${this.balance}`
    );
  }
  ...
  withdraw(amount: number): void {
    if (this.balance > amount) {
      ...
      console.log(
        `Withdrew ${amount} from your account. Your remaining balance: ${this.balance}`
      );
...
class Customer {
  ...
  mobileNumber: number; // Mobile number of the customer
  account: BankAccount; // Bank account of the customer
...
async function interactWithBankAccount() {
  ...
  do {
    ...
    const customer = customers.find(
      (customer) => customer.account.accountNumber === accountNumberInput
    ); // Finding the customer with the entered account number

    if (customer) {
      console.log(chalk.blue("----------------------------------")); // Separator line
      console.log(
        "\t",
        chalk.green(`Welcome, ${customer.firstName} ${customer.lastName}!`)
